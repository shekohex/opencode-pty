---
name: pty-skill
description: Interactive PTY (pseudo-terminal) management for long-running processes. Use when you need to run dev servers, watch modes, REPLs, or any interactive CLI program that requires sending input and reading output over time.
triggers:
  - start a dev server
  - run in background
